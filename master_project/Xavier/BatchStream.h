...
public:
    ...
    {
        FILE* file = fopen(std::string("/home/nvidia/project/baidu_project/tx2/release/data/in8/baidu/batches/batch0").c_str(), "rb");
        int d[4];
        fread(d, sizeof(int), 4, file);
        mDims = nvinfer1::DimsNCHW{d[0], d[1], d[2], d[3]};
        fclose(file);
        mImageSize = mDims.c() * mDims.h() * mDims.w();
        outputSize=mDims.h() * mDims.w()/8/8;
        mBatch.resize(mBatchSize * mImageSize, 0);
        mLabels.resize(mBatchSize * outputSize, 0);
        mFileBatch.resize(mDims.n() * mImageSize, 0);
        mFileLabels.resize(mDims.n() * outputSize, 0);
        reset(0);
    }
    ...
    bool next()
    {
        ...
        for (int csize = 1, batchPos = 0; batchPos < mBatchSize; batchPos += csize, mFileBatchPos += csize)
        {
            ...
            std::copy_n(getFileLabels() + mFileBatchPos * outputSize, csize*outputSize, getLabels() + batchPos*outputSize);
        }
        mBatchCount++;
        return true;
    }
...
private:
    ...
    bool update()
    {
        std::string inputFileName = locateFile(std::string("/home/nvidia/project/baidu_project/tx2/release/data/in8/baidu/batches/batch") + std::to_string(mFileCount++));
        ...
        size_t readLabelCount = fread(getFileLabels(), sizeof(float), mDims.n()*outputSize, file);

        assert(readInputCount == size_t(mDims.n() * mImageSize) && readLabelCount == size_t(mDims.n()*outputSize));
