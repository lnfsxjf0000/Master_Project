...
def val(args, val_loader, model):
    ...
    for i, (input, label, size, name) in enumerate(val_loader):
        ...
        print("[%d/%d]  time: %.4f" % (i + 1, total_batches, time_taken))
...
def train(args, train_loader, model, criterion, optimizer, epoch):
    ...
    for iteration, batch in enumerate(train_loader, 0):
        ...
        print('=====> epoch[%d/%d] iter: (%d/%d) \tcur_lr: %.6f loss: %.3f time:%.4f' % (epoch + 1, args.max_epochs,
...
def train_model(args):
    ...
    if args.dataset == 'camvid':
        ...
        criteria = ProbOhemCrossEntropy2d(use_weight=True, ignore_label=ignore_label,
                                          thresh=0.7, min_kept=min_kept,weight=datas['classWeights'])
    elif args.dataset == 'baidu':
        min_kept = int(args.batch_size // len(args.gpus) * h * w // 16)
        criteria = ProbOhemCrossEntropy2d(use_weight=True, ignore_label=ignore_label,
                                          thresh=0.7, min_kept=min_kept,weight=datas['classWeights'])
    else:
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            # load_state={k: v for k, v in model_checkpoint.items() if k in model_state }
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', default="baidu", help="dataset: cityscapes or camvid")
    ...
    parser.add_argument('--resume', type=str, default="./checkpoint/baidu/DABNetbs8gpu1_train/model_901.pth",
